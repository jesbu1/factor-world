...
from factor_world.envs.asset_path_utils import full_assets_path_for
from factor_world.envs.factors.arm_pos import ArmPosWrapper
from factor_world.envs.factors.camera_pos import CameraPosWrapper
from factor_world.envs.factors.distractor_pos import DistractorPosWrapper
from factor_world.envs.factors.distractor_xml import DistractorXmlWrapper
from factor_world.envs.factors.floor_texture import FloorTextureWrapper
from factor_world.envs.factors.light import LightWrapper
from factor_world.envs.factors.object_pos import ObjectPosWrapper
from factor_world.envs.factors.object_size import ObjectSizeWrapper
from factor_world.envs.factors.object_texture import ObjectTextureWrapper
from factor_world.envs.factors.object_xml import ObjectXmlWrapper
from factor_world.envs.factors.table_pos import TablePosWrapper
from factor_world.envs.factors.table_texture import TableTextureWrapper
from factor_world.envs.factors.xml_utils import get_texture_names
from factor_world.envs.factors.xml_utils import generate_xml
...
def make_env_with_factors(
    ...
    texture_names = get_texture_names(full_assets_path_for(factors_relative_path))

    # Cannot have both distractor_object and object factors, as both
    # require modifying the XML.
    assert not ("distractor_xml" in factor_kwargs and "object_xml" in factor_kwargs)
    if "distractor_xml" in factor_kwargs:
        env = DistractorXmlWrapper(
            env_cls, env_kwargs, **factor_kwargs["distractor_xml"]
        )
        del factor_kwargs["distractor_xml"]
    elif "object_xml" in factor_kwargs:
        env = ObjectXmlWrapper(env_cls, env_kwargs, **factor_kwargs["object_xml"])
