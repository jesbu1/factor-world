...
from factor_world.envs.factors import constants
from factor_world.envs.factors.env_utils import get_table_pos
...
class DistractorPosWrapper(FactorWrapper):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        xyr_low = np.append(xy_low, [theta_range[0]])
        xyr_high = np.append(xy_high, [theta_range[1]])

        self.distractor_init_quat = [1, 0, 0, 0]
        super().__init__(
            env,
            factor_space=spaces.Box(
                low=xyr_low, high=xyr_high, dtype=np.float32, seed=seed
            ),
            **kwargs,
        )

        self.i_qp = self.model.get_joint_qpos_addr(f"joint_{self.distractor_name}")[0]
        self.i_qv = self.model.get_joint_qvel_addr(f"joint_{self.distractor_name}")[0]
    ...
    def _set_to_factor(self, value: Tuple[float, float]):
        ...
        """
        if not hasattr(self, "i_qp") or not hasattr(self, "i_qv"):
